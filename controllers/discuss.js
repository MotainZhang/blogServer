...
class DiscussController {
  static async create(ctx) {
    const validator = ctx.validate(ctx.request.body, {
	  ...
	  isLike:Joi.number().required(),
	  likeNum:Joi.number().required()
    })

    if (validator) {
      try {
        const { articleId, userId, content,isLike,likeNum  } = ctx.request.body
        let commentId = ctx.request.body.commentId

        if (!commentId) {
          // 添加评论
          const comment = await CommentModel.create({ userId, articleId, content,isLike,likeNum })
          commentId = comment.id
        } else {
          // 添加回复
          await ReplyModel.create({ userId, articleId, content, commentId,isLike,likeNum })
        }
