...
async function getAvatarUrl() {
	const result = await axios.get(`https://api.uomg.com/api/rand.avatar?&format=json`)
	return result.data
}
class UserController {
	...
	static async getNews(ctx) {
		...
		try {
			let result = await axios.get(ctx.request.body.url)
			let splitFirst = result.data.replace("artiList(","")
			let splitLast = splitFirst.slice(0,splitFirst.length - 1)
			let objectRes = JSON.parse(splitLast)
			ctx.client(200, '获取成功', objectRes)
		} catch (error) {
			throw error
		}
	}
	// 解析新闻HTml
	static async readerNews(ctx) {
		const {
			url,
		} = ctx.request.body
		try {
			...
			let articleHtml = $('.article-content').html()
			ctx.client(200, '解析成功', articleHtml)
		} catch (error) {
			throw error
		}
	}
	...
	static async githubLogin(ctx, code) {
		try {
			...
			if (access_token) {
				...
				let target = await UserController.find({
					id: githubInfo.id
				}) // 在数据库中查找该用户是否存在
	...
	static async initGithubUser(githubName) {
		const temp = await UserController.find({
			username: githubName
		})
		if (!temp) {
	...
	static async editUser(ctx) {
		...
		if (validator) {
			const encryPassword = await encrypt(ctx.request.body.password)
			ctx.request.body.password = encryPassword;
			await UserModel.update(ctx.request.body, {
				where: {
					id: ctx.request.body.id
				}
			})
